...
The next post will start with some example code from the LFE edition of Casting
SPELs in Lisp, where we wrote a simple looking server which accepted messages.
We will then convert this to OTP using the ``gen_server``  behaviour.
